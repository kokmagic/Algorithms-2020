    ...
    static public String longestCommonSubstring(String first, String second) {
        ...
        for (int i = 0; i < first.length(); i++) {
            for (int j = 0; j < second.length(); j++) {
                boolean equal = first.charAt(i) == second.charAt(j);
                if ((i == 0 || j == 0) && equal) {
                    subLength[i][j] = 1;
                    break;
                }
                ...
                if (subLength[i][j] > maxLength) maxLength = subLength[i][j];
            }
        }
        for (int i = 0; i < first.length(); i++) {
            for (int j = 0; j < second.length(); j++) {
               if (subLength[i][j] == maxLength) {
                   int count = maxLength - 1;
                   while (count >= 0) {
                       substring.append(first.charAt(i - count));
                       count--;
                   }
